      ...
      <div class="row align-items-center position-relative">
        ...
        <div class="col-lg-10 col-sm-9 col-7 d-flex align-items-center justify-content-end position-static">
          <div class="nav-wrapper">
            <ul class="nav">
              <li class="nav-item">
                <a class="{{ request()->routeIs('home') ? 'current-menu-parent' : '' }}" href="{{ route('home') }}">Home</a>
              </li>
              <li class="nav-item">
                <a class="{{ request()->routeIs('games*') ? 'current-menu-parent' : '' }}" href="{{ route('games') }}">Account</a>
              </li>
              <li class="nav-item">
                <a class="{{ request()->routeIs('giveaway*') ? 'current-menu-parent' : '' }}" href="{{ route('giveaway') }}">Give Aways</a>
              </li>
              @if (Auth::check())
              <li class="nav-item">
                <a class="{{ request()->routeIs('profile.referral') ? 'current-menu-parent' : '' }}" href="{{ route('profile.referral') }}">Referral</a>
              </li>
              @endif
              <li class="nav-item">
                <a class="{{ request()->routeIs('contact') ? 'current-menu-parent' : '' }}" href="{{ route('contact') }}">Contact</a>
              </li>
              <li class="nav-item">
                <a onclick="window.location.href = '{{ Auth::check() ? './profile/tickets' : './signin' }}';">Support</a>
              </li>
              <li class="nav-item dropdown">
                ...
                <div class="dropdown-menu" style="width: 270px; line-height:0.6; padding:10px" aria-labelledby="cartDropdown">
                  @if(session('cart'))
                      ...
                      <div class="row">
                        <div class="col-md-4">
                          <div class="item-image">
                            ...
                            @endphp
                            <img width="50" src="{{ $image->main_image }}" alt="{{ $details['name'] }}" />
                          </div>
                        </div>
                        <div class="col-md-8">
              ...
              <li class="nav-item">
                ...
                <ul class="sub-menu">
                  <li>
                    <a>Balance {{ Auth::user()->balance + Auth::user()->referral_balance }}$</a>
                  </li>
                  <li>
              ...
              @else
              <li class="nav-item">
                <a class="{{ request()->routeIs('contact') ? 'current-menu-parent' : '' }} btn btn-primary text-white" href="{{ Auth::check() ? './profile/tickets' : './signin' }}">Sign In</a>
              </li>
              @endif
            </ul>
          </div>
      ...
      <ul class="navbar-nav">
        ...
        <li class="nav-item dropdown">
          ...
          <div class="dropdown-menu" aria-labelledby="categoryDropdown{{ $category->id }}">
            ...
            <a class="dropdown-item" href="{{ route('games.category', ['slug' => $subCategory->name]) }}">{{ $subCategory->name }}</a>
            @endforeach

            <!-- SubSubcategories for the current subcategory -->
            @foreach ($subCategory->subSubCategory as $subSubCategory)
            <a class="dropdown-item" href="{{ route('games.subcategory', ['slug' => $subSubCategory->name]) }}">{{ $subSubCategory->name }}</a>
            @endforeach
          </div>
        </li>
        @endforeach
