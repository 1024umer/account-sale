...
{{-- <script>
  $(document).ready(function () {
      // Initial hide of the clicked state container
      $('.confirmresult.clicked').hide();

      $('#acceptCheckbox').on('change', function () {
          var isChecked = $(this).prop('checked');

          if (isChecked) {
              $('.confirmresult.clicked').show();
              $('.simpleResult').hide();

              var data = JSON.parse(`{!! addslashes($finalResult) !!}`);

              var content = '';
              for (var key in data) {
                  content += `<strong>${key}:</strong> ${data[key]}<br>`;
              }
              $('.confirmresult.clicked').html(content);
          } else {
              $('.simpleResult').show();
              // Hide the clicked state container
              $('.confirmresult.clicked').hide();
          }
      });
  });
                            ...
                            <div class="pay-div p-4 pt-2">
                                ...
                                @foreach ($product->emailChannels as $emailChannel)
                                    @if ($emailChannel->status == 'available')
                                        ...
                                        <button data-bs-toggle="modal" data-bs-target="#staticBackdrop" class="w-100 payBtn my-2">Go To Pay</button>
                            ...
                            <div class="col-md-6 left-side p-4">
                                ...
                                <div class="bottom p-2">
                                    <div class="d-flex my-3 align-items-center justify-content-between">
                                        ...
                                        <h1>$<span class="totalPrice">{{ $product->price - ($product->price * $product->discount / 100) }}</span>
                            ...
                            <div class="col-md-6 right p-4">
                                <div class="top p-2">
                                    ...
                                    <div class="icons-div my-4">
                                        ...
                                        @endif
                                        @if ($user != null)
                                            @if ($product->price - ($product->price * $product->discount / 100) < $user->balance)
