...
class GamingAccountController extends Controller
{
  ...
  public function add(Request $request)
  {
    ...
    $gamingAccount->sub_subcategory_id = $request['sub_subcategory']??Null;
    ...
    if ($stock_list_str != '') {
      ...
      if (count($stock_list) == 1) {
        ...
        if ($request->additional_format && $request->additional_stock_list) {
          $additionalFormats = $request->input('additional_format', []);
          $additionalStockLists = $request->input('additional_stock_list', []);

          foreach ($additionalFormats as $key => $additionalFormat) {
            $additionalEmailChannel = new EmailChannel();
            $additionalEmailChannel->format = $additionalFormat;
            $additionalEmailChannel->value1 = $additionalStockLists[$key];
            $additionalEmailChannel->delimiter = $stock_delimiter;
            $additionalEmailChannel->status = "available";

            $gamingAccount->emailChannels()->save($additionalEmailChannel);
          }
        }
      } else {
